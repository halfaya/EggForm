...
open import Data.String         public using (String; intersperse; words; lines) renaming (_++_ to _++s_; _==_ to _==s_; toList to toChars; fromList to fromChars)
open import Data.Product        public using (_×_; _,_; Σ; uncurry; swap) renaming (proj₁ to fst; proj₂ to snd)
open import Data.Unit           public using (⊤; tt)
open import Data.Vec            public using (Vec; []; _∷_; lookup; _[_]%=_; toList; fromList; updateAt; head; last; foldr₁; zipWith) renaming (concat to cat; replicate to rep; map to vmap; _∷ʳ_ to _v∷ʳ_; zip to vzip; reverse to vreverse; _++_ to _++v_; take to vtake; drop to vdrop)
